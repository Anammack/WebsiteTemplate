...
All design tokens live at the top of `css/style.css` in the `:root` block.
...
The form currently shows a fake success message after 1.2 s.
To make it real, find the `TODO` block in `js/main.js → initContactForm()`
...
No IE11 support — uses CSS custom properties and IntersectionObserver.
